...
| # | PRD | 实际决策 | 状态 |
|---|-----|----------|------|
| C1 | 全端共用「微信登录/手机号登录」账号体系（§4.2.7） | local-first：**无登录墙**，仅可选关联手机号/微信 | **用户已裁：改 PRD，维持无登录墙**（2026-09-23） |
| C2 | 小程序 P0 与 MVP 并行（§4.2.8） | 架构里程碑把小程序壳放 MVP，功能可后置；T3.1–3.2 已补壳 | **用户已裁：改 PRD，维持壳优先**（2026-09-23） |
| C3 | 年报「自动生成」（§2.6.2） | 实现为**用户手动触发**生成 | **用户已裁：暂不改功能**；PRD 已注明「临时手动，待补原版」（2026-09-23） |
| C4 | SF-36 标准计分 | 实现 0–4 简化计分供自管趋势 | **用户已裁：补齐标准 SF-36 计分** → worklist G7 |
